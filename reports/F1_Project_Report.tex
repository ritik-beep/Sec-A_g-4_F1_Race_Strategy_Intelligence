...
PPT \& Quality Lead  & Palaparthi Harshakarthikeya \\
\bottomrule
\end{tabular}\\[1cm]
...
The Ergast database encodes missing values as the string literal \texttt{\textbackslash N}. All 14 CSVs were loaded with \texttt{dtype=str} to preserve these sentinel values, which were subsequently replaced with \texttt{pd.NA} and cast to appropriate data types.
...
\textbf{Model:} $\text{points} \sim \text{grid} + \text{stop\_count} + \text{avg\_pit\_ms}$

\begin{itemize}[noitemsep]
    \item $R^2 = 0.60$ --- the model explains 60\% of points variance, which is robust for sports analytics.
    \item Grid position is the strongest predictor ($\beta \approx -0.6$, standardised): each grid position lost reduces expected points by $\sim$0.6.
    \item Pit stop duration is significant ($p < 0.05$) but with a smaller effect size.
    \item Stop count shows a non-linear relationship (2-stop is optimal at Strategy-Dominant circuits, but 1-stop is optimal at Qualifying-Dominant circuits).
\end{itemize}
...
\textbf{Features:} \texttt{avg\_delta}, \texttt{avg\_qualifying\_gap\_ms}, \texttt{lap\_time\_std}\\
\textbf{Output:} Three distinct circuit archetypes:
...
Mixed               & $0.5$--$0.7$ & Most other circuits & Read race conditions. Adapt based on safety car. \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{Cluster distribution:} 5 Qualifying-Dominant, 10 Strategy-Dominant, 19 Mixed circuits (among circuits with sufficient data).
...
\midrule
Mitul Bhatia             & $\bullet$ & $\bullet$ &           &           &           & $\bullet$ & $\bullet$ \\
